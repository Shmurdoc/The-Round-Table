...
@section('title', $cohort->title ?? $cohort->name . ' - RoundTable')
...
<div class="slide-up space-y-6">
    ...
    <div class="relative h-64 rounded-3xl overflow-hidden shadow-2xl border-4 border-slate-900/10">
        ...
        <div class="absolute bottom-0 left-0 right-0 p-8 z-10">
            ...
            <h1 class="text-3xl md:text-4xl font-extrabold text-white drop-shadow-lg">{{ $cohort->title ?? $cohort->name }}</h1>
        </div>
    </div>
    @endif
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.cohorts.edit', $cohort) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-amber-100 text-amber-700 text-sm font-bold rounded-xl hover:bg-amber-200 transition">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            <div class="text-2xl font-bold text-slate-900 font-mono">{{ $stats['total_members'] ?? $cohort->members_count ?? 0 }}</div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            <div class="text-2xl font-bold text-slate-900 font-mono">R{{ number_format(($cohort->current_capital ?? 0) / 100, 0) }}</div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            <div class="text-2xl font-bold text-slate-900 font-mono">R{{ number_format(($cohort->contribution_amount ?? 0) / 100, 0) }}</div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
            ...
            <div class="text-2xl font-bold text-slate-900 font-mono">{{ $stats['distributions_count'] ?? 0 }}</div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div class="p-6">
            <div class="mb-6">
                <div class="text-sm text-slate-600 mb-4">
                    ...
                    <span class="px-3 py-1 text-xs font-bold rounded-full {{ 
                        $cohort->status === 'draft' ? 'bg-slate-100 text-slate-700' : 
            ...
            <form action="{{ route('admin.cohorts.change-status', $cohort) }}" method="POST" class="space-y-4">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        <option value="draft" {{ $cohort->status === 'draft' ? 'selected' : '' }}>Draft (Private)</option>
                        <option value="funding" {{ $cohort->status === 'funding' ? 'selected' : '' }}>Funding (Open for Members)</option>
                        <option value="operational" {{ $cohort->status === 'operational' ? 'selected' : '' }}>Operational (Active)</option>
                        <option value="paused" {{ $cohort->status === 'paused' ? 'selected' : '' }}>Paused</option>
    ...
    <div class="bg-gradient-to-br from-amber-50 to-amber-100 border border-amber-200 rounded-2xl p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <form action="{{ route('admin.cohorts.activate-production', $cohort) }}" method="POST" onsubmit="return confirm('Are you sure you want to activate production mode? This will notify all partners and begin weekly tracking.');">
                    @csrf
    ...
    <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-2xl p-6">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-emerald-700">Activated {{ $cohort->production_activated_at->format('F d, Y \a\t g:i A') }}</p>
            </div>
        </div>
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-6">
                ...
                <form action="{{ route('admin.cohorts.timeline.store', $cohort) }}" method="POST" enctype="multipart/form-data" class="p-5 space-y-4">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="date" name="event_date" value="{{ date('Y-m-d') }}" required class="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="divide-y divide-slate-100 max-h-[600px] overflow-y-auto">
                    @forelse($cohort->timelines()->latest('event_date')->latest('created_at')->limit(20)->get() as $timeline)
                        <div class="p-5 hover:bg-slate-50 transition-colors">
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 {{ $timeline->event_type_color }} rounded-xl flex items-center justify-center flex-shrink-0">
                                    <i data-lucide="{{ $timeline->event_type_icon }}" class="w-5 h-5 text-white"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-4 mb-2">
                                        <div class="flex-1">
                                            <h4 class="font-bold text-slate-900 text-sm mb-1">{{ $timeline->title }}</h4>
                                            <div class="flex items-center gap-2 text-xs text-slate-500">
                                                <span>{{ $timeline->event_date->format('M d, Y') }}</span>
                                                <span>•</span>
                                                <span>{{ $timeline->created_at->diffForHumans() }}</span>
                                                @if($timeline->profit_amount > 0)
                                                    <span>•</span>
                                                    <span class="font-bold text-emerald-600">R{{ number_format($timeline->profit_amount / 100, 2) }}</span>
                                                @endif
                                            </div>
                                        </div>
                                        <span class="text-[10px] font-bold uppercase px-2 py-1 rounded {{ $timeline->event_type === 'profit' ? 'bg-emerald-100 text-emerald-700' : ($timeline->event_type === 'alert' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700') }}">
                                            {{ str_replace('_', ' ', $timeline->event_type) }}
                                        </span>
                                    </div>
                                    <p class="text-sm text-slate-600 mb-3">{{ $timeline->description }}</p>
                                    @if($timeline->proof_document)
                                        <a href="{{ Storage::url($timeline->proof_document) }}" target="_blank" class="inline-flex items-center gap-2 text-xs text-purple-600 hover:text-purple-700 font-bold">
                                            <i data-lucide="paperclip" class="w-3 h-3"></i>
                                            View Proof Document
                                        </a>
                                    @endif
                                </div>
                                <form action="{{ route('admin.cohorts.timeline.destroy', [$cohort, $timeline]) }}" method="POST" onsubmit="return confirm('Delete this timeline entry?');" class="flex-shrink-0">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-5 space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-600">{{ $cohort->description }}</p>
                    </div>
                    @if($cohort->exit_strategy)
                        <div>
                            <div class="text-[10px] uppercase font-bold text-slate-400 mb-1">Exit Strategy</div>
                            <p class="text-sm text-slate-600">{{ $cohort->exit_strategy }}</p>
                        </div>
                    @endif
                    @if($cohort->funding_end_date)
                        <div>
                            <div class="text-[10px] uppercase font-bold text-slate-400 mb-1">Funding Deadline</div>
                            <p class="text-sm text-slate-900">{{ $cohort->funding_end_date->format('M d, Y') }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div class="p-4 space-y-2">
                    <a href="{{ route('admin.cohorts.control-panel', $cohort) }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                    ...
                    <a href="{{ route('admin.cohorts.profits.index', $cohort) }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                    ...
                    <a href="{{ route('admin.cohorts.votes.create', $cohort) }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                    ...
                    <a href="{{ route('admin.cohorts.reports.create', $cohort) }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
                    ...
                    <a href="{{ route('admin.cohorts.edit', $cohort) }}" class="flex items-center space-x-3 p-3 rounded-xl hover:bg-slate-50 transition-colors group">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="p-5 border-b border-slate-100 flex items-center justify-between">
                    ...
                    <span class="text-xs text-slate-500">{{ $cohort->members_count ?? 0 }} total</span>
                </div>
                        ...
                        <div class="p-4 hover:bg-slate-50 transition-colors">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3 flex-1">
                                    ...
                                    <div class="flex-1">
                                        <div class="font-bold text-slate-900 text-sm">{{ $member->user->name ?? 'Unknown' }}</div>
                                        <div class="text-xs text-slate-500">Joined {{ $member->created_at->format('M d, Y') }}</div>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    <div class="text-right">
                                        <div class="text-sm font-bold text-slate-900 font-mono">R{{ number_format($member->capital_paid / 100, 2) }}</div>
                                        <div class="text-xs text-slate-500">
                                            {{ $cohort->current_capital > 0 ? number_format(($member->capital_paid / $cohort->current_capital) * 100, 1) : 0 }}% stake
                                        </div>
                                    </div>
                                    <span class="text-[10px] font-bold uppercase px-2 py-1 rounded
