...
<div class="slide-up space-y-6 max-w-4xl mx-auto">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p class="text-slate-500 text-sm mt-2">Review {{ $user->name }}'s identity documents</p>
        </div>
        <a href="{{ route('admin.kyc') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-200 transition">
        ...
        <div class="p-6">
            <div class="flex items-center gap-6 mb-6">
                <div class="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center">
                    <span class="text-2xl font-bold text-slate-600">{{ strtoupper(substr($user->name ?? 'U', 0, 1)) }}</span>
                </div>
                <div>
                    <h2 class="text-xl font-bold text-slate-900">{{ $user->name }}</h2>
                    <p class="text-slate-500">{{ $user->email }}</p>
                    <p class="text-sm text-slate-400">Submitted {{ $user->kyc_submitted_at?->diffForHumans() ?? 'recently' }}</p>
                </div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <span class="text-xs text-slate-400 uppercase font-bold">Phone</span>
                    <p class="text-slate-900 font-medium">{{ $user->phone ?? 'Not provided' }}</p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 uppercase font-bold">ID Number</span>
                    <p class="text-slate-900 font-mono">{{ $user->kyc_id_number ?? 'Not provided' }}</p>
                </div>
                <div>
                    ...
                    @endphp
                    <span class="inline-flex px-3 py-1 text-xs font-bold rounded-lg {{ $statusColors[$user->kyc_status] ?? $statusColors['not_started'] }}">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                @if($user->kyc_id_document_front)
                    <div class="bg-slate-50 rounded-xl p-4 border border-slate-200">
                        <a href="{{ Storage::url($user->kyc_id_document_front) }}" target="_blank" 
                           class="inline-flex items-center gap-2 text-amber-600 hover:text-amber-700 font-medium">
                            <i data-lucide="external-link" class="w-4 h-4"></i>
                            View Document
                        </a>
                        <img src="{{ Storage::url($user->kyc_id_document_front) }}" 
                             alt="ID Front" 
            ...
            <div>
                ...
                @if($user->kyc_id_document_back)
                    <div class="bg-slate-50 rounded-xl p-4 border border-slate-200">
                        <a href="{{ Storage::url($user->kyc_id_document_back) }}" target="_blank" 
                           class="inline-flex items-center gap-2 text-amber-600 hover:text-amber-700 font-medium">
                            <i data-lucide="external-link" class="w-4 h-4"></i>
                            View Document
                        </a>
                        <img src="{{ Storage::url($user->kyc_id_document_back) }}" 
                             alt="ID Back" 
                             class="mt-3 max-h-64 rounded-lg border border-slate-200">
                    </div>
                @else
            ...
            <div>
                ...
                @if($user->kyc_proof_of_residence)
                    <div class="bg-slate-50 rounded-xl p-4 border border-slate-200">
                        <a href="{{ Storage::url($user->kyc_proof_of_residence) }}" target="_blank" 
                           class="inline-flex items-center gap-2 text-amber-600 hover:text-amber-700 font-medium">
                            <i data-lucide="external-link" class="w-4 h-4"></i>
                            View Document
                        </a>
                        <img src="{{ Storage::url($user->kyc_proof_of_residence) }}" 
    ...
    @if($user->kyc_status === 'pending')
        ...
        <div id="rejectModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
            <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl">
                ...
                <form action="{{ route('admin.kyc.reject', $user) }}" method="POST" class="p-6">
                    @csrf
                    <div class="mb-4">
        ...
        <div class="bg-red-50 border border-red-200 rounded-2xl p-6">
            <div class="flex items-start gap-4">
                ...
                <div>
                    ...
                    <p class="text-red-700 mt-1">{{ $user->kyc_rejection_reason ?? 'No reason provided.' }}</p>
                </div>
            </div>
        </div>
