...
<div class="slide-up">
    ...
    <div class="flex flex-col md:flex-row justify-between items-end mb-8 space-y-4 md:space-y-0">
        ...
        <div class="flex space-x-2 bg-white p-1 rounded-lg border border-slate-200 shadow-sm">
            <a href="<?php echo e(route('cohorts.index')); ?>" 
               class="px-3 py-1.5 text-xs font-bold <?php echo e(!request('status') ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'); ?> rounded shadow-sm transition-colors">
                All
            </a>
            <a href="<?php echo e(route('cohorts.index', ['status' => 'funding'])); ?>" 
               class="px-3 py-1.5 text-xs font-bold <?php echo e(request('status') === 'funding' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'); ?> rounded transition-colors">
                Open
            </a>
            <a href="<?php echo e(route('cohorts.index', ['status' => 'operational'])); ?>" 
               class="px-3 py-1.5 text-xs font-bold <?php echo e(request('status') === 'operational' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'); ?> rounded transition-colors">
                Active
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pb-20">
        <?php $__empty_1 = true; $__currentLoopData = $cohorts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cohort): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <?php
               ...
               class="group bg-white rounded-2xl border border-slate-200 hover:border-amber-300 shadow-sm hover:shadow-xl hover:shadow-amber-900/5 hover:-translate-y-1 transition-all duration-300 cursor-pointer overflow-hidden flex flex-col h-full relative">
                ...
                <div class="relative h-48 overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900">
                    ...
                    <div class="absolute top-4 left-4 right-4 flex justify-between items-start z-10">
                        <span class="px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider rounded-lg border backdrop-blur-sm <?php echo e($riskLevel['color']); ?> bg-white/90">
                            <?php echo e($riskLevel['label']); ?>
                        ...
                        <div class="flex items-center space-x-2 px-3 py-1.5 bg-slate-900/80 backdrop-blur-sm rounded-lg border border-white/10">
                            ...
                            <span class="text-xs font-mono text-white"><?php echo e($cohort->duration_months ?? 6); ?>M</span>
                        </div>
                    </div>
                    ...
                    <div class="absolute bottom-4 left-4 z-10">
                        <span class="px-3 py-1 text-xs font-bold text-white bg-amber-500/90 backdrop-blur-sm rounded-lg">
                            <?php echo e(ucfirst(str_replace('_', ' ', $cohort->asset_type ?? 'real_estate'))); ?>
                ...
                <div class="p-6 flex flex-col flex-1">
                    <div class="mb-1">
                        <span class="text-[10px] font-bold text-amber-600 uppercase tracking-widest"><?php echo e($cohortType); ?></span>
                    </div>
                    ...
                    <p class="text-sm text-slate-500 line-clamp-3 mb-6 flex-1 leading-relaxed">
                        <?php echo e(Str::limit($cohort->description, 150)); ?>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                            ...
                            <div class="text-sm font-mono font-bold text-slate-900">R<?php echo e(number_format($cohort->ideal_target / 100, 0)); ?>k</div>
                        </div>
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                            <div class="text-[10px] text-slate-400 uppercase font-bold">Est. Yield</div>
                            <div class="text-sm font-mono font-bold text-emerald-600"><?php echo e($cohort->projected_annual_return ?? '12-18'); ?>%</div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between text-xs items-end">
                            <div>
                                <span class="text-slate-900 font-bold"><?php echo e(number_format($percent, 0)); ?>%</span>
                                <span class="text-slate-400">funded</span>
                            </div>
                            <span class="text-slate-400 text-[10px]"><?php echo e($memberCount); ?>/<?php echo e($maxMembers); ?> seats</span>
                        </div>
                        <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden relative">
                            <!-- MVC Marker -->
                            <div class="absolute top-0 bottom-0 w-0.5 bg-slate-300 z-10" style="left: <?php echo e($mvcPercent); ?>%"></div>
                            <div class="bg-slate-900 h-full rounded-full transition-all duration-1000 ease-out" style="width: <?php echo e(min($percent, 100)); ?>%"></div>
                        </div>
                    </div>
                </div>
    ...
    <!-- Pagination -->
    <?php if($cohorts->hasPages()): ?>
        <div class="flex justify-center pb-10">
            <?php echo e($cohorts->links()); ?>
...
<?php echo $__env->make('layouts.modern', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\The round table\The Table\resources\views/cohorts/index-modern.blade.php ENDPATH**/ ?>