...
<?php $__env->startSection('title', 'Manage Cohorts - RoundTable'); ?>
<?php $__env->startSection('page-title', 'My Cohorts'); ?>
...
<div class="slide-up space-y-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <a href="<?php echo e(route('admin.cohorts.create')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 text-slate-900 font-bold rounded-xl hover:bg-amber-400 transition shadow-lg shadow-amber-200">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php $__empty_1 = true; $__currentLoopData = $cohorts ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cohort): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-lg transition-all group">
                ...
                <div class="p-6">
                    ...
                    <?php endif; ?>
                    <h3 class="font-bold text-slate-900 text-lg mb-1 line-clamp-1"><?php echo e($cohort->title ?? $cohort->name); ?></h3>
                    <p class="text-sm text-slate-500 mb-4 line-clamp-2"><?php echo e($cohort->description); ?></p>
                    
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <div class="text-[10px] uppercase font-bold text-slate-400">Members</div>
                            <div class="font-bold font-mono text-slate-900"><?php echo e($cohort->members_count ?? 0); ?></div>
                        </div>
                        <div>
                            <div class="text-[10px] uppercase font-bold text-slate-400">Capital</div>
                            <div class="font-bold font-mono text-slate-900">R<?php echo e(number_format(($cohort->current_capital ?? 0) / 100, 0)); ?></div>
                        </div>
                    </div>
                    
                    <div class="flex gap-2">
                        <a href="<?php echo e(route('admin.cohorts.show', $cohort)); ?>" class="flex-1 px-4 py-2 bg-slate-100 text-slate-700 text-sm font-bold rounded-lg text-center hover:bg-slate-200 transition">
                            View
                        </a>
                        <a href="<?php echo e(route('admin.cohorts.edit', $cohort)); ?>" class="flex-1 px-4 py-2 bg-amber-100 text-amber-700 text-sm font-bold rounded-lg text-center hover:bg-amber-200 transition">
                            Edit
                        </a>
                    </div>
                </div>
        ...
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
            <div class="col-span-full bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
                ...
                <a href="<?php echo e(route('admin.cohorts.create')); ?>" class="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 text-slate-900 font-bold rounded-xl hover:bg-amber-400 transition">
                    <i data-lucide="plus" class="w-5 h-5"></i>
                    Create Cohort
                </a>
            </div>
    ...
    <?php if(isset($cohorts) && $cohorts->hasPages()): ?>
...
<?php echo $__env->make('layouts.modern', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\The round table\The Table\resources\views/admin/cohorts/index.blade.php ENDPATH**/ ?>