...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'RoundTable | Cohort Capital'); ?></title>
...
<body class="bg-slate-50 text-slate-900 overflow-x-hidden antialiased selection:bg-amber-200 selection:text-amber-900 bg-noise font-sans">
    ...
    <main class="transition-all duration-300 md:ml-72 p-4 md:p-10 pb-28 md:pb-10 min-h-screen">
        ...
        <header class="flex justify-between items-center mb-10 sticky top-0 bg-slate-50/90 backdrop-blur-md z-30 py-4 -mx-4 px-4 md:-mx-10 md:px-10 border-b border-transparent transition-all" id="main-header">
            <div class="flex items-center space-x-3 text-sm font-medium text-slate-400">
                ...
                <span class="text-slate-900 font-bold capitalize bg-white/50 px-3 py-1 rounded-md border border-slate-200/50 shadow-sm"><?php echo $__env->yieldContent('page-title', 'Dashboard'); ?></span>
            </div>
            ...
            <div class="flex items-center space-x-3 pl-4">
                ...
                <div class="flex items-center space-x-3 bg-white p-1.5 pr-4 rounded-full border border-slate-200 shadow-sm cursor-pointer hover:border-amber-400/30 hover:shadow-md transition-all group" id="user-menu-btn">
                    <div class="h-9 w-9 rounded-full bg-slate-900 text-amber-400 flex items-center justify-center font-bold text-xs ring-2 ring-white group-hover:ring-amber-100 transition-all">
                        <?php echo e(strtoupper(substr(auth()->user()->first_name ?? 'U', 0, 1))); ?><?php echo e(strtoupper(substr(auth()->user()->last_name ?? 'N', 0, 1))); ?>

                    </div>
                    <div class="hidden sm:block text-xs text-right leading-tight">
                        <div class="font-bold text-slate-900"><?php echo e(auth()->user()->first_name ?? 'User'); ?> <?php echo e(auth()->user()->last_name ?? ''); ?></div>
                        <div class="text-slate-500 font-mono text-[10px]">ID: #<?php echo e(str_pad(auth()->user()->id, 4, '0', STR_PAD_LEFT)); ?></div>
                    </div>
                </div>
            </div>
        </header>
    ...
    <div id="user-dropdown" class="hidden fixed top-20 right-4 md:right-10 bg-white rounded-xl shadow-2xl border border-slate-200 w-64 z-[60] fade-in">
        <div class="p-4 border-b border-slate-100">
            <div class="text-sm font-bold text-slate-900"><?php echo e(auth()->user()->first_name); ?> <?php echo e(auth()->user()->last_name); ?></div>
            ...
            <div class="mt-2">
                <span class="text-[10px] font-bold uppercase px-2 py-1 rounded-md 
                    <?php if(in_array(auth()->user()->kyc_status, ['approved', 'verified'])): ?> bg-emerald-100 text-emerald-700
                    <?php elseif(auth()->user()->kyc_status === 'pending'): ?> bg-amber-100 text-amber-700
                    <?php else: ?> bg-slate-100 text-slate-600 <?php endif; ?>">
                    KYC: <?php echo e(ucfirst(auth()->user()->kyc_status ?? 'Not Started')); ?>

                </span>
            </div>
        </div>
        <div class="p-2">
            <a href="<?php echo e(route('kyc.form')); ?>" class="flex items-center space-x-3 px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
                <i data-lucide="user-circle" class="w-4 h-4"></i>
                <span>Profile & KYC</span>
            </a>
            <a href="<?php echo e(route('member.portfolio')); ?>" class="flex items-center space-x-3 px-3 py-2.5 text-sm text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
    ...
    <div id="notification-dropdown" class="hidden fixed top-20 right-4 md:right-10 bg-white rounded-xl shadow-2xl border border-slate-200 w-96 max-h-[600px] overflow-y-auto z-[60] fade-in">
        <div class="sticky top-0 bg-white border-b border-slate-100 p-4 z-10">
            <div class="flex items-center justify-between">
                ...
                <?php if($unreadCount > 0): ?>
                <span class="text-xs font-bold px-2 py-1 bg-amber-100 text-amber-700 rounded-md"><?php echo e($unreadCount); ?> new</span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="divide-y divide-slate-100">
            ...
            <a href="<?php echo e($notification->action_url ?? '#'); ?>" 
               class="block p-4 hover:bg-slate-50 transition-colors <?php echo e($notification->read ? 'opacity-60' : 'bg-amber-50/30'); ?>">
                <div class="flex items-start gap-3">
                    <div class="flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center
                        <?php echo e($notification->type === 'kyc' ? 'bg-emerald-100' : 
                           ($notification->type === 'distribution' ? 'bg-purple-100' : 
                           ($notification->type === 'cohort' ? 'bg-blue-100' : 'bg-slate-100'))); ?>">
                        <i data-lucide="<?php echo e($notification->type === 'kyc' ? 'shield-check' : 
                                          ($notification->type === 'distribution' ? 'banknote' : 
                                          ($notification->type === 'cohort' ? 'layers' : 'bell'))); ?>" 
                           class="w-5 h-5 
                           <?php echo e($notification->type === 'kyc' ? 'text-emerald-600' : 
                              ($notification->type === 'distribution' ? 'text-purple-600' : 
                              ($notification->type === 'cohort' ? 'text-blue-600' : 'text-slate-600'))); ?>"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-slate-900 mb-1"><?php echo e($notification->title); ?></p>
                        <p class="text-xs text-slate-600 line-clamp-2"><?php echo e($notification->message); ?></p>
                        <div class="flex items-center gap-2 mt-2">
                            <span class="text-[10px] text-slate-400 font-mono"><?php echo e($notification->created_at->diffForHumans()); ?></span>
        ...
        <div class="sticky bottom-0 bg-slate-50 border-t border-slate-100 p-3">
            <a href="<?php echo e(route('member.notifications')); ?>" class="block text-center text-xs font-bold text-amber-600 hover:text-amber-700 transition-colors">
...
<?php /**PATH C:\wamp64\www\The round table\The Table\resources\views/layouts/modern.blade.php ENDPATH**/ ?>